...
class ContextTest < Minitest::Test
  ...
  def test_role_method
    ...
    Context.define name do
      ...
      def empty_method #see test_simple for at least one method for context
      end
    end
  ...
  def test_role_method_args
    ...
    Context.define name do
      ...
      def empty_method # see test_simple
      end
    end
    ...
    Context.define name do
      ...
      role role_name do
        def rolem(*args, &b)
          ...
          args.each { |x|
            # bind :x=>:num
            @num = x # temporary fix, until bind start to work. See test_bind
