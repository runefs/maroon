...
interpretation_context.role_aliases[key] = value end
  
def self_expressions_empty? 
(expressions == ImmutableQueue.empty) end
  
def self_potential_bind_is_bind? 
potential_bind and (potential_bind.length and ((potential_bind[0] == :call) and ((potential_bind[1] == nil) and (potential_bind[2] == :bind)))) end
...
self_potential_bind_is_bind? and self_block_rewrite end
  
def self_block_rewrite 
changed = false
argument_list = potential_bind[3]
if argument_list and (argument_list[0] == :arglist) then
  ...
  if arguments and (arguments[0] == :hash) then
    ...
    (1..count).each do |j|
      ...
      unless aliased_role.instance_of?(Symbol) and interpretation_context.roles.has_key?(aliased_role) then
        raise("#{aliased_role} used in binding is an unknown role #{roles}")
      end
