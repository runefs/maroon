...
@@with_contracts = value
 ...
 def name_from_def(definition) if definition[1].instance_of?(Symbol) then
  definition[1]
else
  ((definition[1].select { |e| e.instance_of?(Symbol) }.map { |e| e.to_s }.join(".") + ".") + definition[2].to_s).to_sym
end end
 def create_info(definition) raise("Sexp require") unless definition.instance_of?(Sexp)
...
if @defining_role and (not sources) then
 ...
 def add_method(exp) name = name_from_def(exp)
sources = get_methods(name)
(sources << exp)
 end
 def finalize(file_path,with_contracts) raise("No name") unless @name
...
if file_path then
 ...
 def generate_context_code() getters = ""
impl = ""
interactions = ""
