class Context
  ...
  def current_interpretation_context(*args, &b)
    ...
    InterpretationContext.new(@roles, @contracts, @role_alias, @defining_role)
  end
  ...
  def generate_context_code()
    ...
    @interactions.each do |method_name, methods|
      methods.each do |method|
        @defining_role = nil
        info = MethodInfo.new method,method_name,@private
        code = info.build_as_context_method(current_interpretation_context)
        raise "No code produced" if code == nil
        code = " " + code
    ...
    @roles.each do |role, methods|
      ...
      methods.each do |method_name, method_sources|
        ...
        @defining_role = role
        info = MethodInfo.new method_source,"self_" + role.to_s + "_" + method_name.to_s,@private
